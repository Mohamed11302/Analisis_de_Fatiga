...
class BBT(JUEGO):
    ...
    def normalizar_serie(self):
        valor_max = self.fatiga_serie
        valor_min = self.fatiga_serie
        hijo = self.hijo 
        while hijo != None:
            _max = hijo.fatiga_serie
            _min = hijo.fatiga_serie
            if valor_max < _max:
                valor_max = _max
            if valor_min > _min:
                valor_min = _min
        ...
        hijo = self.hijo
        while hijo != None:
            fatiga_serie_normalizado = round(((hijo.fatiga_serie)-valor_min) / (valor_max - valor_min), 3)
            hijo.fatiga_serie = fatiga_serie_normalizado
            hijo = hijo.hijo
        fatiga_serie_normalizado = round(((self.fatiga_serie)-valor_min) / (valor_max - valor_min), 3)
    ...
    def extraer_datos_comparacion(self):
        ...
        if self.hijo == None:
            ...
            while hijo != None:
                ...
                datos_historico[hijo.date] = self.calcular_media_comparacion(data_inicio_reps, hijo.datos_comparacion)
                hijo = hijo.hijo
            datos_historico = self.calcular_media_historica(datos_historico)
            self.datos_comparacion = data_inicio_reps
            self.datos_comparacion = self.calcular_media_comparacion(datos_historico, data_inicio_reps)
    ...
    def calcular_media_comparacion(self, datos_historico, data_inicio_reps):
        datos_comparacion = {}
        for clave, valor in datos_historico.items():
            if isinstance(valor, (int, float)):
                datos_comparacion[clave] = (datos_historico[clave]+data_inicio_reps[clave])/2
            if isinstance(valor, dict):
                datos_comparacion[clave] = {}
                for subclave, _ in valor.items():
                    datos_comparacion[clave][subclave] = (datos_historico[clave][subclave]+ data_inicio_reps[clave][subclave])/2
